...
template <typename InType, typename OutType, cudaDataType_t DCuSPARSE>
void BmFN(std::string benchmark, int dimK, int vec_length, int kernel, bool sorted, bool func, int sparse){
    ...
    printf("%.2f,",1.0-(float)nonzeros/(m*n));
    std::cout << vec_length << "," << m_vec << "," << m << "," << k << "," << n << ",";
	...
	int NUM_PROFILES = 512;
        ...
        cudaProfilerStop();
	std::cout << "vectorSparse SpMM runtime " << spmm_ms_avg/(float)NUM_PROFILES; 
	...
	for(int iter=0; iter<NUM_PROFILES; ++iter){
        ...
        cudaProfilerStop();
	std::cout << "cusparse fp16 block ell runtime " << dense_ms_avg/(float)NUM_PROFILES;
	...
	std::cout << dense_ms_avg/(float)NUM_PROFILES;
        // Functional verification
        if (func){
            float *output_value_host = new float[A_num_rows * k];
            // traverse all the vector rows
...
int main(int argc, char **argv){
    ...
    // Run the benchmark
        ...
        if (mixed) BmFN<half, half, CUDA_R_16F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse);
        else BmFN<float, float, CUDA_R_32F>(benchmark, dimK, vec_length, kernel, sorted, func, sparse); 
    }
    printf("\n");
}
